#!/bin/bash
# Start track node, support RViz point selection to set target initial position (UAV version)

echo "=== Starting Track Node (UAV Target) ==="
echo ""
...
cleanup() {
    ...
    if [ -f "$FAKE_TARGET_PID_FILE" ]; then
        ...
        if [ ! -z "$FAKE_TARGET_PID" ] && kill -0 "$FAKE_TARGET_PID" 2>/dev/null; then
            echo "  Terminating fake_target.launch (PID: $FAKE_TARGET_PID)"
...
echo "  Waiting 30 seconds, if no point selected use default position (x=2, y=0, z=2)"
...
if [ "$POINT_RECEIVED" = true ]; then
    ...
    if [ ! -z "$X" ] && [ ! -z "$Y" ]; then
        TARGET_X="$X"
        TARGET_Y="$Y"
        echo "  Selected point position: x=$TARGET_X, y=$TARGET_Y, z=2 (fixed)"
    else
...
echo "  Target position: x=$TARGET_X, y=$TARGET_Y, z=2"
...
echo ""
echo "3. Launching fake_target.launch..."
...
echo ""
echo "=== Track started ==="
echo "Target position: ($TARGET_X, $TARGET_Y, 2)"
