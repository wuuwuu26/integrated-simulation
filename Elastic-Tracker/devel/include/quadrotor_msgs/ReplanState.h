  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::quadrotor_msgs::ReplanState_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.iniState.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.iniState[i]);
    }
    if (v.iniState.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.path.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.path[i]);
    }
    if (v.path.empty() || true)
      s << "]";
    if (true || !indent.empty())
